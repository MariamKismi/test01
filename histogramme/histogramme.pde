...
void draw() {
  ...
  int xstart = int(constrain(mouseX - w/2, 0, width));
  int ystart = int(constrain(mouseY -w/2, 0, height-200));
  int xend = int(constrain(mouseX + w/2, 0, width));
  int yend = int(constrain(mouseY + w/2, 0, height-200));
  ...
  for (int x = xstart; x < xend; x++) {
    for (int y = ystart; y < yend; y++ ) {
      histoR[constrain(int(red(get(x, y))), 0, 255)]++;
      histoG[constrain(int(green(get(x, y))), 0, 255)]++;
      histoB[constrain(int(blue(get(x, y))), 0, 255)]++;
    }
  }
  noFill();
  stroke(255, 255,255);
